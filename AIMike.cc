...
struct PLAYER_NAME : public Player {
  ...
  void bfs_analitzar(int myid){
    ...
    while (not Q.empty()){
        ...
        if (cellid != -1 and unit(cellid).type == Zombie){
          pair <Dir,Dir> dir_ficha = desfer_cami(p, act, Prev);
          estrategia_zombie(myid, p, distance[p.i][p.j], dir_ficha);
          return;
        }

        if (cellid != -1 and unit(cellid).player != me() and unit(cellid).type == Alive){
          pair <Dir,Dir> dir_ficha = desfer_cami(p, act, Prev);
        ...
        for (int i = 0; i < 4; ++i){
          ...
          if (pos_movible(new_pos) and distance[new_pos.i][new_pos.j] == -1){
              Q.push(new_pos);
